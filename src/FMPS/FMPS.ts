...
export class FMPS {
  ...
  async match_auto_update(root: string) {
    ...
    let arry = [];
    let nicname = await this.ctx.http.get(
      "https://1145141919810-1317895529.cos.ap-chengdu.myqcloud.com/json%2Fsms_studata_main.json",
    );
    try {
      for (let i = 0; i < studata_cn.length; i++) {
        let nic: string[] = [];
        let twtxt: string;
        let alias_cn = this.complete_alias(
          studata_cn[i].Name.replace(/（/g, "(").replace(/）/g, ")"),
        );
        let alias_in = this.complete_alias(
          studata_tw[i].Name.replace(/（/g, "(").replace(/）/g, ")"),
        );
        let nameEn = studata_cn[i].PathName;
        //暂时性的繁简互换方案
        if (
          studata_jp[i].Name == studata_tw[i].Name ||
          studata_tw[i].Name == ""
        ) {
        ...
        arry.push({
          ...
          Name_zh_cn: studata_cn[i].Name.replace(/（/g, "(").replace(
            /）/g,
            ")",
          ),
          Name_zh_ft: studata_zh[i].Name.replace(/（/g, "(").replace(
            /）/g,
            ")",
          ),
          NickName: nic,
  ...
  async sanae_match_refinement(root: string) {
    ...
    try {
      ...
      const regex = /\(([^)]+)\)/g;
      const supplementary_name = await this.json_parse(
        root + "/sms_to_arona_data_revisions.json",
      );
      let ss = 0;
      for (let i = 0; i < studata_zh.length; i++) {
        const db_name = studata_zh[i].Name.replace(/（/g, "(").replace(
          /）/g,
          ")",
        );
        ...
        if (supplementary_name[ss].Id == (10000 + i).toString()) {
          ...
          ss == supplementary_name.length - 1
            ? (ss = supplementary_name.length - 1)
            : ss++;
        }
        arrys.push({
    ...
    fs.writeFile(
      `${root}/sms_studata_toaro_stu.json`,
      jsonString,
      (err: any) => {
        if (err) {
          logger.info("数据更新时发生错误", error);
          ...
          const endTime = new Date().getTime(); // 获取结束时间的毫秒表示
          logger.info(
            "sanae_match_to_arona_data数据生成完毕,用时" +
  ...
  async name_detection(sms_arona, type) {
    ...
    try {
      for (let i = 0; i < sms_arona.length; i++) {
        const url =
          "https://arona.diyigemt.com/api/v2/image?name=" + sms_arona[i][key];
      ...
      if (errnum != 0) {
        logger.info(
          "⚠️ sanae_match_to_arona_data数据检测完毕，发现" +
            errnum +
            "个问题,用时" +
            (endTime - startTime) * 0.001 +
            "秒",
        ...
        logger.info(
          "🟢 sanae_match_to_arona_data数据检测完毕，未发现问题,用时" +
            (endTime - startTime) * 0.001 +
            "秒",
        );
      }
  ...
  async img_to_channel(data: Buffer, appId, secret, channelId) {
    async function refreshToken(bot) {
      const { access_token: accessToken, expires_in: expiresIn } =
        await this.ctx.http.post("https://bots.qq.com/app/getAppAccessToken", {
          appId: bot.appId,
          clientSecret: bot.secret,
        });
      bot.token = accessToken;
      await this.ctx.setTimeout(
        () => refreshToken.bind(this)(bot),
        (expiresIn - 30) * 1000,
      );
    }
    ...
    if (typeof data === "string") {
      ...
      throw new Error("Blob not supported here");
    ...
    await this.ctx.http.post(
      `https://api.sgroup.qq.com/channels/${bot.channelId}/messages`,
      payload,
      {
        headers: {
  ...
  async guide_download_image(
    ...
    if (fs.existsSync(imagePath)) {
      ...
      for (let i = 1; i < 4; i++) {
        try {
          // 下载图片
          const response = await this.ctx.http.get(imageUrl, {
            responseType: "arraybuffer",
          });
          ...
          const status = error.response ? error.response.status : "无法获取";
          logger.info(
            `文件下载出现错误，进行第${i}次尝试: Error: HTTP error! status: ${status}`,
          );
          if (i === 3) {
            logger.info(`${i}次尝试后依旧出错😭`);
          }
